...
The Upstox V3 collector subscribes to the 500-stock source universe plus NIFTY and India VIX. At 08:30 IST the engine keeps at most 250 F&O stocks meeting volume, daily-range, spread and support/resistance filters. NIFTY is classified as `TRENDING`, `RANGE`, `HIGH_VOL` or `TRANSITION` from ADX, VIX, ATR% and breadth. Only VWAP pullback continuation in `TRENDING` and range-extreme mean reversion in `RANGE` may enter; ORB is a fallback requiring more than 2× first-five-minute volume. Risk is capped at 0.25% of capital and ₹1,000 including modeled costs, with 1.5R–2R targets, one open position and four trades daily. The ₹3,000 target and 0.8%-of-capital daily loss breaker stop new entries.
...
Both OCI execution services and their start timers remain disabled while v4 awaits current-session deployed evidence. Equity signal checks are configured for 60 seconds and completed-candle exit checks for 30 seconds, with a 60-second minimum hold for thesis exits. Hard stops remain protective; break-even begins at 0.75R and trailing at 1R with a 1.5×ATR distance. Options code is retained but its execution path is bypassed and sandbox submission disabled. Live trading remains prohibited.
