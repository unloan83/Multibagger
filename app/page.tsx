...
type IndiaModel = "breeze-multibagger" | "upstox-intraday" | "options-quant";
...
export default function HomePage() {
  ...
  const [activeModel, setActiveModel] = useState<IndiaModel>("breeze-multibagger");

  useEffect(() => {
    const requested = new URLSearchParams(window.location.search).get("tab");
    if (requested === "breeze-multibagger" || requested === "upstox-intraday" || requested === "options-quant") setActiveModel(requested);
      ...
      <div className="mx-auto max-w-[1500px] space-y-5">
        <header className="rounded-2xl border border-slate-800 bg-[#091322] p-5 shadow-xl">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
              <div className="flex flex-wrap items-center gap-2"><h1 className="text-xl font-bold tracking-tight text-white">Production Trading Models</h1><span className="rounded-full border border-emerald-500/25 bg-emerald-500/10 px-2.5 py-1 text-[10px] font-bold text-emerald-300">DEVELOPMENT GATE ACTIVE</span></div>
