...
const DATA_FILE_PATH = upstoxDataPath("upstox_recommendations.json");

import { scoreUpstoxCandidateTriage, TriageLevel, UpstoxTriageDetails } from "@/features/upstox/lib/upstox-market-triage";
...
export type UpstoxRec = {
  ...
  remark: string;
  timestamp: string;
  // Market Intelligence Triage indicators (simple UI & backend details)
...
function readData(): { asOf: string; sandboxMode: boolean; defaultExecutionMode: string; recommendations: UpstoxRec[] } {
  ...
  if (Array.isArray(store.recommendations)) {
    store.recommendations = store.recommendations.map((rec: UpstoxRec) => {
      if (!rec.triageDetails || !rec.triageScore) {
        const details = scoreUpstoxCandidateTriage({
          symbol: rec.symbol,
          name: rec.name,
        ...
        rec.triageDetails = details;
        rec.triageScore = details.triageScore;
        rec.marketAlignment = details.marketAlignment;
        rec.smartMoney = details.smartMoney;
        rec.expertConsensus = details.expertConsensus;
      }
      return rec;
    });
  }
...
export async function getUpstoxRecommendations() {
  try {
    const data = readData();
    const now = Date.now();
    let modified = false;
...
export async function updateUpstoxRecommendations(request: Request) {
  try {
