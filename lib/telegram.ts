...
export async function sendTelegramInteractiveAlert({
  chatId,
  text,
  recommendationId,
  symbol,
  ...
  const response = await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
    ...
    body: JSON.stringify({
      ...
      reply_markup: {
        inline_keyboard: [
          [
            { text: `🛒 Buy ${symbol} (Sandbox)`, callback_data: `upstox:buy:${recommendationId}` },
            { text: `🔻 Sell ${symbol} (Sandbox)`, callback_data: `upstox:sell:${recommendationId}` },
            { text: "⏭️ Skip", callback_data: `upstox:skip:${recommendationId}` },
          ]
        ]
      }
    }),
...
export function buildDailyTelegramDigest({
  ...
  portfolioRecords: DigestRecord[];
  marketRecords: DigestRecord[];
  dateLabel: string;
}) {
  ...
  return [
    `Multibagger Daily Picks — ${dateLabel}`,
...
function latestBySymbol(records: DigestRecord[]) {
...
function isBuyRecommendation(record: DigestRecord) {
  return /^(buy|accumulate)$/iu.test(record.action.trim());
}

function isIntradayRecommendation(record: DigestRecord) {
  return /intraday|today|minute|hour/iu.test(`${record.section} ${record.horizon}`);
}

function formatRecommendationLines(records: DigestRecord[], emptyMessage: string) {
...
// Token encryption/decryption helpers (kept for secure token storage)
